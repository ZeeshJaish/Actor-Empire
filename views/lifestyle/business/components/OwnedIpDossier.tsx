...
const getIpTypePresentation = (type: OwnedRight['propertyType']) => {
    if (type === 'CHARACTER') return { Icon: Users, tone: 'border-sky-400/35 bg-sky-400/10 text-sky-300' };
    if (type === 'STORY_WORLD') return { Icon: Globe, tone: 'border-violet-400/35 bg-violet-400/10 text-violet-300' };
    if (type === 'FRANCHISE') return { Icon: Layers, tone: 'border-amber-400/35 bg-amber-400/10 text-amber-300' };
    return { Icon: Archive, tone: 'border-teal-400/35 bg-teal-400/10 text-teal-300' };
};
...
export const OwnedIpDossier: React.FC<OwnedIpDossierProps> = ({
    ...
    const projectsValue = isStudioOriginal
        ? tr('ownedIp.projects.released', { count: performance.releases.length })
        : ownedRight.projectsAllowed === undefined
        ? tr('ownedIp.projects.made', { count: ownedRight.projectsUsed })
        : tr('ownedIp.projects.left', { count: Math.max(0, ownedRight.projectsAllowed - ownedRight.projectsUsed) });

    return (
        <div
            role="dialog"
            aria-modal="true"
            aria-label={tr('ownedIp.dossier.aria', { title: ownedRight.title })}
        ...
        >
                ...
                <header className="sticky top-0 z-20 border-b border-white/10 bg-[#08080b] px-5 pb-4 pt-[max(1rem,env(safe-area-inset-top))]">
                    <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                            <p className="text-[8px] font-black uppercase tracking-[0.24em] text-amber-300">{tr('ownedIp.dossier.title')}</p>
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-2">
                                ...
                                <span className={`inline-flex items-center gap-1 rounded border px-2 py-1 text-[7px] font-black uppercase tracking-[0.12em] ${ipType.tone}`}>
                                    <IpTypeIcon size={11} /> {tr(`ownedIp.type.${ownedRight.propertyType}`)}
                                </span>
                                <span className="inline-flex items-center gap-1 text-[7px] font-black uppercase tracking-[0.14em] text-emerald-300">
                                    <ShieldCheck size={11} /> {isStudioOriginal ? tr('ownedIp.ownership.studioOriginal') : tr('ownedIp.ownership.owned')}
                                </span>
                            </div>
                        </div>
                        <button
                            type="button"
                            onClick={onClose}
                            aria-label={tr('ownedIp.dossier.closeAria')}
                ...
                <main className="space-y-7 px-5 py-5">
                    <section>
                        <div className="mb-3 flex items-center gap-2">
                            ...
                            <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-300">{tr('ownedIp.section.ownership')}</h3>
                        </div>
                        <div className="grid grid-cols-2 divide-x divide-y divide-white/10 overflow-hidden rounded-xl border border-white/10 bg-white/[0.025] sm:grid-cols-4 sm:divide-y-0">
                            {[
                                [tr('ownedIp.ownership.control'), isStudioOriginal ? tr('ownedIp.ownership.studioOriginal') : formatLabel(ownedRight.dealType)],
                                [tr('ownedIp.ownership.timeline'), isPermanent ? tr('ownedIp.ownership.permanent') : tr('ownedIp.ownership.weeks', { weeks: weeksLeft || 0 })],
                                [tr('ownedIp.ownership.projects'), projectsValue],
                                [isStudioOriginal ? tr('ownedIp.ownership.created') : tr('ownedIp.ownership.acquired'), isStudioOriginal ? tr('ownedIp.ownership.inHouse') : formatMoney(ownedRight.purchasePrice)],
                            ].map(([label, value]) => (
                    ...
                    <section>
                        <div className="mb-3 flex items-center gap-2">
                            ...
                            <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-300">{tr('ownedIp.section.expansion')}</h3>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            >
                                ...
                                <span className="mt-3 block text-[9px] font-black uppercase tracking-[0.13em] text-white">{tr('ownedIp.action.franchiseCommand')}</span>
                                <span className="mt-1 block text-[8px] leading-relaxed text-zinc-600">{tr('ownedIp.action.franchiseDescription')}</span>
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                ...
                                <span className="mt-3 block text-[9px] font-black uppercase tracking-[0.13em] text-white">{tr('ownedIp.action.universeCommand')}</span>
                                <span className="mt-1 block text-[8px] leading-relaxed text-zinc-600">{tr('ownedIp.action.universeDescription')}</span>
                            </button>
                        </div>
                        <div className="mt-2 flex gap-2">
                            ...
                            >
                                <Clock size={13} /> {tr('ownedIp.action.holdIp')}
                            </button>
                            {renewalQuote.available ? (
                                ...
                                >
                                    <RefreshCw size={13} /> {tr('ownedIp.action.renewLicence', { amount: formatMoney(renewalQuote.cost) })}
                                </button>
                            ) : null}
                        </div>
                        {renewalFeedback ? (
                    ...
                    <section>
                        <div className="mb-3 flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-300">{tr('ownedIp.performance.title')}</h3>
                            </div>
                            <span className="text-[7px] font-black uppercase tracking-[0.15em] text-zinc-700">{tr('ownedIp.performance.existingReleaseData')}</span>
                        </div>
                        <div className="grid grid-cols-4 divide-x divide-white/10 border-y border-white/10 bg-white/[0.02] py-3">
                            <div className="min-w-0 px-2 first:pl-0">
                                <p className="text-[6px] font-black uppercase tracking-[0.13em] text-zinc-600">{tr('ownedIp.performance.lifetimeGross')}</p>
                                {metricValue(performance.releases.length ? formatMoney(performance.lifetimeGross) : tr('ownedIp.performance.unproven'), performance.lifetimeGross > 0)}
                            </div>
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2">
                            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-3">
                                <p className="text-[7px] font-black uppercase tracking-[0.14em] text-zinc-600">{tr('ownedIp.performance.audienceStrength')}</p>
                            ...
                            <div className={`rounded-xl border px-3 py-3 ${momentum.surface}`}>
                                <p className="text-[7px] font-black uppercase tracking-[0.14em] text-zinc-600">{tr('ownedIp.performance.momentum')}</p>
                    ...
                    <section>
                        <div className="mb-3 flex items-center gap-2">
                            ...
                            <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-300">{tr('ownedIp.pipeline.title')}</h3>
                        </div>
            ...
            <footer className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-[#08080b] px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3">
                <div className="mx-auto flex w-full max-w-3xl gap-2">
                        ...
                        >
                            {tr('ownedIp.action.backToLibrary')}
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            {isStudioOriginal ? tr('ownedIp.action.openFranchiseCommand') : tr('ownedIp.action.developIp')}
                        </button>
                </div>
            </footer>
        </div>
