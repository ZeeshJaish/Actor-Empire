        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto space-y-6">
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full bg-zinc-950 border rounded-lg px-4 py-3 text-white focus:outline-none transition-colors ${titleError ? 'border-rose-500/60 focus:border-rose-400' : 'border-zinc-800 focus:border-blue-500'}`}
                        />
                        <div className="mt-2 flex items-start justify-between gap-4">
                            <p className={`text-xs ${titleError ? 'text-rose-400' : 'text-zinc-500'}`}>
                                {titleError || 'You can rename this project in the Script Vault until Greenlight.'}
                            </p>
                            <span className="shrink-0 text-[10px] font-mono text-zinc-600">
                                {normalizeProjectTitle(title).length}/{PROJECT_TITLE_MAX_LENGTH}
                            </span>
                        </div>
                    </div>
                ...
                <div className="p-6 border-t border-zinc-800 bg-zinc-900/50 flex justify-end gap-3">
                    ...
                    <button 
                        onClick={() => !titleError && setStep('CONFIRM')}
                        disabled={!!titleError || (!useOriginalWriter && !selectedWriter)}
