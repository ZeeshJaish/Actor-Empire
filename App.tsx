...
import { INITIAL_PLAYER, Player, Page, Commitment, PressInteraction, SocialEvent, ActorSkills, AdType, Relationship } from './types';
...
export const App: React.FC = () => {
  ...
  const syncCurrentSlotSnapshot = (nextPlayer: Player) => {
      ...
      saveGameData(`actorEmpireSave_${currentSlot}`, nextPlayer);
  };
  ...
  useEffect(() => {
    const init = async () => {
        try {
            ...
            // 2. Migration: If no IDB data in any slot, check legacy localStorage
            ...
            if (!hasAnySave) {
                ...
                if (legacySave) {
                    try {
                        ...
                        await saveGameData('actorEmpireSave_1', savedData);
                        slots[1] = savedData;
  ...
  useEffect(() => {
    if (gameStatus === 'PLAYING' && currentSlot) {
        ...
        saveGameData(`actorEmpireSave_${currentSlot}`, player);
    }
  ...
  useEffect(() => {
    if (gameStatus === 'PLAYING') {
      setPlayer(prev => {
          ...
          // Ensure Arrays
          if (!safePlayer.activeReleases) safePlayer.activeReleases = [];
          if (!safePlayer.pastProjects) safePlayer.pastProjects = [];
  ...
  const handleAdComplete = (type: AdType, data?: any) => {
      ...
      handleGenericUpdate(prev => {
          ...
          if (type === 'REWARDED_CASH') {
              ...
              // DYNAMIC BAILOUT: 20% of Debt + 5k
              const currentDebt = Math.abs(p.money < 0 ? p.money : 0);
              const bailoutAmount = Math.floor(currentDebt * 0.20) + 5000;
              ...
              p.energy.current = Math.min(100, p.energy.current + 25);
              toastSub = "+25 Energy";
          }
                      ...
                      <>
                          ...
                          <div className="space-y-3">
                              ...
                              <button 
                                  ...
                                  className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg"
                              >
                                  <PlayCircle size={18}/> Get Bailout (+${(Math.floor(Math.abs(player.money) * 0.20) + 5000).toLocaleString()})
                              </button>
                ...
                <div className={`flex-1 p-5 overflow-y-auto custom-scrollbar ${player.money < 0 ? 'pt-8' : ''}`}>
                    ...
                    {activePage === Page.MOBILE && (
                        <MobilePage 
                            ...
                            onAudition={(opp)=>handleGenericUpdate(p=>({ ...p, energy: { ...p.energy, current: p.energy.current-25 }, applications: [...p.applications, { id: `app_${Date.now()}`, type: 'AUDITION', name: opp.projectName, weeksRemaining: 1, data: opp }] }))}
                            ...
                            onPost={(t,c,img)=>handleGenericUpdate(p=>{
                                ...
                                return { 
                                    ...
                                    energy: { ...p.energy, current: p.energy.current-10 }, 
                            ...
                            onPerformSponsorship={(id, type)=>handleGenericUpdate(p=>{ const s = p.activeSponsorships.find(x=>x.id===id); return s ? { ...p, energy: { ...p.energy, current: p.energy.current-s.requirements.energyCost } } : p; })}
