...
assert(OWNED_STREAMING_PLATFORM_SCHEMA_VERSION === 26, 'The canonical foundation should preserve Technology Campus in schema v25.');
const migrated = normalizeOwnedStreamingPlatformState({ schemaVersion: 14 }, 'phase17-migration');
assert(migrated.schemaVersion === 26 && migrated.technologyProjects.length === 0, 'Schema v14 saves should migrate with an empty technology-project history.');
