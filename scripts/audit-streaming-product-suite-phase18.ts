...
assert(OWNED_STREAMING_PLATFORM_SCHEMA_VERSION === 27, 'The canonical foundation should preserve Product Lab in schema v27.');
const migrated = normalizeOwnedStreamingPlatformState({ schemaVersion: 15 }, 'phase18-migration');
assert(migrated.schemaVersion === 27 && migrated.productLines.length === 0, 'Schema v15 saves should migrate with a safe empty product portfolio.');
