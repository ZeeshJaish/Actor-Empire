...
assert(OWNED_STREAMING_PLATFORM_SCHEMA_VERSION === 26, 'The canonical foundation should advance owned streaming saves to schema v25.');
const migrated = normalizeOwnedStreamingPlatformState({ schemaVersion: 17 }, 'phase20-migration');
assert(migrated.schemaVersion === 26, 'Schema v17 streaming saves should migrate to the current schema.');
