...
assert.equal(automaticallySigned.delegationTrace?.mandateRevision, 1, 'delegated package signing must retain its exact mandate revision');
assert.equal(automaticallySigned.delegationTrace?.rule, 'CATALOGUE_PACKAGE_WITHIN_MANDATE');
assert.equal(automaticallySigned.delegationTrace?.facts.packageId, automaticallySigned.id);
assert.ok(typeof automaticallySigned.delegationTrace?.facts.selectedPlatformId === 'string');
