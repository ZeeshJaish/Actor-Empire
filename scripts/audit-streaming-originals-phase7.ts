...
assert(OWNED_STREAMING_PLATFORM_SCHEMA_VERSION === 26, 'The current schema should retain the Phase 7 Original and slate records.');
const migrated = normalizeOwnedStreamingPlatformState({ schemaVersion: 5 }, 'legacy-phase7');
assert(migrated.schemaVersion === 26, 'Phase 5 saves should normalize to the current schema.');
